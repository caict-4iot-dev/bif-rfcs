...
在原始私钥前添加地址编码类型前缀，目前支持的地址编码类型请参见[BID规范](#jump)中介绍的编码类型。

注意：此地址编码类型为BID地址编码类型，非生成私钥的最后一步Base58编码类型，两者并无直接相关关系。

**第三步，添加前缀加密类型**

上一步生成的结果基础上添加前缀加密类型，目前支持的加密编码类型请参见[BID规范](#jump)中介绍的加密类型。
...
在原始公钥前添加地址编码类型前缀，目前支持的地址编码类型请参见[BID规范](#jump)中介绍的编码类型。
...
| IPV6     | 16字节 | 2<sup>128</sup> | 128  | 1/2<sup>128</sup> | 1亿          | 130EH/s  | 4.4*10<sup>10</sup>年  |
