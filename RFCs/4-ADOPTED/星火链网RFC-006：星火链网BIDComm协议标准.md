...
在基于星火链网标识符BID的交互中，需要一套标准的通信协议，描述通信双方如何以BID为数据载体，进行数据交互和信息通信。同时，基于该标准协议，BID的基本认证流程（BIDAuth）、可信证书（BID-VC）等交互流程的规范流程同样需要定义。因此，本RFC定义BIDComm协议标准和交互流程。
