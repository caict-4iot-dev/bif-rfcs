...
BIDComm提供了节点之间的交互通道，建立通信后，节点之间需要相互认证，认证的过程称之为BIDAuth， 这篇RFC阐释了BIDAuth的工作流程和相关规范。
# 4. 原理
...
在BID数字世界中，需要借助于密码学知识去进行各方身份的相互验证，在BID文档中，通过指定相应的机制来实现数字世界的身份认证。建立通信通道，BIDAuth身份认证后，在此基础上进行授权、可验证凭据等功能实现（上层功能并不在本RFC讨论范围）

## 4.2 BIDAuth介绍

BIDAuth描述了用户对某数字身份BID拥有控制权，用户只需证明自己拥有该数字身份对应的私钥，可以使用私钥解密出该数字身份对应的公钥所加密的密文的过程。

经过BIDAuth认证之后，不同数字身份之间建立通信通道，后续在此基础上进行授权、可验证凭据等功能实现。

BIDAuth与其它身份验证方法类似，依赖于“挑战-应答”方式，在本协议中，结合BID的特性，制定了“挑战-应答”的认证过程。
